...
export default function NumericalIndicatorsComponent(props: NumericalIndicators) {
    ...
    <Card title="Числовые показатели" bordered={false} style={{ width: '100%' }}>
      <div className={styles.cardItemListWrapper}>
        ...
        <div className={styles.cartItemWrapper}>
          <span className={styles.cardItem}>Среднее количество дней изготовления: {prepareRenderValue(props.averageProductionDays)}</span>
        </div>
        <div className={styles.cartItemWrapper}>
          <span className={styles.cardItem}>Количество изделий: {prepareRenderValue(props.numberOfProducts)} шт.</span>
        </div>
      </div>
    </Card>
	)
